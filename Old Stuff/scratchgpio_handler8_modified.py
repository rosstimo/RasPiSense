...
import Adafruit_DHT #added by Tim R
...
#            SH = SenseHat()
	...
	print "I made it to line 3042  Tim R!!!!!"
        global firstRun, cycle_trace, step_delay, stepType, INVERT, \
            Ultra, ultraTotalInUse, piglow, PiGlow_Brightness, compass, ADDON, \
            meVertical, meHorizontal, meDistance, host, killList, socketB, UH #, sgh_pigpio #weird error Tim R
            ...
            for dataItem in dataList:
                ...
                # print "loggin level",debugLogging
                    ...
                    if self.bFindValue('link'):
                        try:
                            self.scratch_socket2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                            self.scratch_socket2.connect((self.value, 42002))
                            print self.scratch_socket2
                    ...
                    if self.bFindValue("getdht11"):
                        print "Yo Mamma !!!"
                        #if sgh_pigpio is None:       	#Removed by Tim R
                        #	sgh_pigpio = pigpio.pi()	#Removed by Tim R
                        #pi = pigpio.pi()	#added by Tim R
                        pin = int(self.valueNumeric) if self.valueIsNumeric else 11
                        #gpiopin = sghGC.gpioLookup[pin]
                        #sensor = dht11.DHT11(sgh_pigpio, gpiopin) #old
                        sensor = Adafruit_DHT.DHT11 #new Tim R
                        ...
                        #dummy = sensor.read()
                        bcast_str = 'sensor-update "%s" %s' % ("temperature", temperature)
                        print 'sending: %s' % bcast_str
                        msgQueue.put(((5, bcast_str)))
                        bcast_str = 'sensor-update "%s" %s' % ("humidity", humidity)
                        print 'sending: %s' % bcast_str
                        msgQueue.put(((5, bcast_str)))
    ...
    def run(self):
        ...
        while not self.stopped():
            ...
            if priority == 11:
                ...
                try:
                    self.scratch_socket.send(b + cmd)
                    print "msg sent to Scratch", str(b), cmd
                except:
